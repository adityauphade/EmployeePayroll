...
var EmpFilterList;
var LoginAdminCredentials = {}
var count = 0;
var baseURL = "http://localhost:3000/EmployeePayrollList";
var emailDropDownTag = document.getElementById("adminEmail")
var numberDropDownTag = document.getElementById("adminPhone")

createTable();

function setDropDownItems(){
    //GET & SET VALUES FOR DROPDOWN
    console.log("dropdown")

    LoginAdminCredentials = JSON.parse(localStorage.getItem('loginKey'));
    emailDropDownTag.innerText = `Email: ${LoginAdminCredentials.email}`;
    numberDropDownTag.innerText = `Phone: ${LoginAdminCredentials.number}`;
}
    ...
    makeRequest('get', baseURL, true).then(data => {
        ...
        employeeObjList.forEach(obj => {
            ...
            ${obj.gender}
                ...
                ${obj.department.map(e => {
                    return `<span class='rowItem'>${e}</span>`
                }).join("")}
                </td>
                <td class="e-salary">
                $ ${obj.salary}
